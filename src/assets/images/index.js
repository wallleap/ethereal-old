export default {
  sakura: 'https://cdn.jsdelivr.net/gh/chanshiyucx/yoi/blog/sakura.png',
  reimu: 'https://cdn.jsdelivr.net/gh/chanshiyucx/yoi/blog/reimu.png',
  preview: {
    touhou: 'https://cdn.jsdelivr.net/gh/wallleap/cdn@latest/assets/img/ethereal_light.png',
    school: 'https://cdn.jsdelivr.net/gh/wallleap/cdn@latest/assets/img/ethereal_dark.png',
  },
  bg: {
    touhou: [
      'https://cdn.jsdelivr.net/gh/wallleap/cdn@latest/assets/img/bg.jpg',
      'https://cdn.jsdelivr.net/gh/wallleap/cdn@latest/assets/img/bg.jpg',
    ],
    school: [
      'https://cdn.jsdelivr.net/gh/wallleap/cdn@latest/assets/img/bg.jpg',
      'https://cdn.jsdelivr.net/gh/wallleap/cdn@latest/assets/img/bg.jpg',
    ],
  },
}
